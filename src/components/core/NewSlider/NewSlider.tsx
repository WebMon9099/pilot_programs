  ...
  return (
    <div
      ...
      className={appendClass(
        'flex items-center p-[50px_20px_0px_20px] text-[#dfdfdf]',
        rest.className
      )}
    >
      <span className="mr-[12px] font-inter text-[12px] font-medium text-[#bbb]">
        {min}
      </span>
      <Holdable
        className="relative my-[5px] h-full flex-1"
        onHold={() => setHold(true)}
      ...
      >
        ...
        />
        <div
          className="absolute bottom-full mb-4 -translate-x-1/2 whitespace-nowrap rounded-full border border-[#e9e9e9] bg-white p-[4px_12px]"
          style={{
            boxShadow: 'rgb(235, 235, 235) 0px 0px 2px 0px',
            left: `${((value - min) / (max - min)) * 100}%`,
          }}
        >
          <span className="font-inter text-[14px] font-semibold text-[#6b6b6b]">
            {value}
          </span>{' '}
          <span className="text-[8px] text-[#b2b2b2]">%</span>
        </div>
      </Holdable>
      <span className="ml-[12px] font-inter text-[12px] font-medium text-[#bbb]">
        {max}
      </span>
    </div>
  );
